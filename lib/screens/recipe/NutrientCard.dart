...
class NutrientCard extends StatefulWidget {
  final recipeId;
  NutrientCard(this.recipeId);

  @override
  _NutrientCardState createState() => _NutrientCardState(recipeId);
}

class _NutrientCardState extends State<NutrientCard> {
  String recipeId = '';
  int servings = 0;
  List<Nutrient> nutrients = List<Nutrient>();

  _NutrientCardState(this.recipeId);

  @override
  Widget build(BuildContext context) => Card(
        child: Column(
          children: <Widget>[
            buildCardHeadline('Nährtoffe'),
            FutureBuilder(
              future: Future.wait([
                Firestore.instance
                    .collection('recipes')
                  ...
                  case ConnectionState.done:
                    if (snapshot.data[0].documents.length == 0) {
                      ...
                      snapshot.data[0].documents.forEach((document) {
                        ...
                        nutrient.name = document['name'];
                        nutrient.amount = document['amount'];
                        nutrient.unit = document['unit'];
                        nutrients.add(nutrient);
                      });
                      if (servings == 0) {
                        DocumentSnapshot document5 =
                            snapshot.data[1].documents.firstWhere((document4) {
                          return document4.documentID == recipeId;
                        });
                        servings = document5['servings'];
                      }

                      List<Widget> nutrientList = List<Widget>();
                      nutrients.forEach((nutrient) {
                          ...
                          Row(
                            children: <Widget>[
                              Container(
                                ...
                                child: Text(
                                  (servings * nutrient.amount).toString() +
                                      nutrient.unit,
                        ...
                        nutrientList.add(Container(height: 8.0));
                      });
